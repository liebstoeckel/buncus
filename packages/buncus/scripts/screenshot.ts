...
const disc = mock.store.createDiscussion({ repositoryId: repo.id, categoryId: cat.id, title: "guide/start", body: "seed\n\n<!-- sha1: seed -->" });
const c1 = mock.store.addComment(disc.id, mock.store.viewerUserId, "Hello from **buncus** — comments hosted from a single binary. Try `inline code` and a [link](https://example.com).");
...
resetConfig();
setConfig({
  publicUrl: "http://localhost:0", apiHost: mock.url, oauthHost: mock.url,
  appId: mock.store.appId, clientId: mock.store.clientId, clientSecret: mock.store.clientSecret,
  privateKey: privateKey.export({ type: "pkcs1", format: "pem" }).toString(),
  encryptionPassword: "shot-password-shot-password-shot-password", dbPath: ":memory:", origins: [], originsRegex: [],
});
...
const browser = await chromium.launch({ channel: "chromium", headless: true, args: ["--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage"] });

async function shot(theme: string, file: string, load: boolean) {
  const page = await browser.newPage({ viewport: { width: 820, height: 900 }, colorScheme: theme === "dark" ? "dark" : "light" });
